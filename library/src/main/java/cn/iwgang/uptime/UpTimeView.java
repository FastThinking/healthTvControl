 ...
 */
public class UpTimeView extends View {
    ...
    public UpTimeView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UpTimeView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mCustomCountDownTimer = new CustomCountDownTimer(millisecond, countDownInterval) {
            ...
            public void onFinish() {
                ...
                if (null != mOnCountdownEndListener) {
                    mOnCountdownEndListener.onEnd(UpTimeView.this);
                }
            }
        };
    ...
    public void updateShow(long millisUntilFinished) {
        ...
        if (mInterval > 0 && null != mOnCountdownIntervalListener) {
            if (mPreviousIntervalCallbackTime == 0) {
                mPreviousIntervalCallbackTime = millisUntilFinished;
            } else if (millisUntilFinished + mInterval <= mPreviousIntervalCallbackTime) {
                mPreviousIntervalCallbackTime = millisUntilFinished;
    ...
    private void reSetTime(long millisUntilFinished) {
        int day = 0;
        int hour;

        if (!mCountdown.isConvertDaysToHours) {
            day = (int) (millisUntilFinished / (1000 * 60 * 60 * 24));
            hour = (int) ((millisUntilFinished % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        } else {
            hour = (int) (millisUntilFinished / (1000 * 60 * 60));
        }
        int allowTime = (int) (millisUntilFinished / (1000 * 60));
        if (allowTime > Config.allowMinute) {
            ...
            if (minute >= Config.alertInternalMinute) {
                EventBus.getDefault().post(new TimeLimit());
    ...
    public interface OnCountdownEndListener {
        void onEnd(UpTimeView cv);
    }

    public interface OnCountdownIntervalListener {
        void onInterval(UpTimeView cv, long remainTime);
    }
